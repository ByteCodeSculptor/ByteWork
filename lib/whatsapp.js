...
/**
 * Build the pre-filled WhatsApp text for a promo post. Uses the niche's own
 * `message` when supplied, else a standard mentorship-framed default, and
 * appends a comment-keyword reference for attribution.
 */
export function buildPromoMessage({ message, serviceName, keyword }) {
  const base =
    message ||
    `Hi ByteWork! I'm interested in the *${serviceName}* final-year project — reference build + source code + 1:1 mentorship so I can learn it and defend it myself.`;
  return keyword ? `${base}\n(Ref: ${keyword})` : base;
}
